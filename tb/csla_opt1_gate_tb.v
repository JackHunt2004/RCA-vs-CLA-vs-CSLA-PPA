...
module csla_opt1_gate_tb;
    ...
    initial begin
        ...
        $dumpfile({"results/power/csla_opt1_gate_", workload, ".vcd"});
        $dumpvars(0, csla_opt1_gate_tb);
        ...
        if (errors == 0)
            $display("PASS: CSLA OPT1 gate-level simulation completed with no errors.");
        else
            $display("FAIL: CSLA OPT1 gate-level simulation found %0d errors.", errors);
