...
module rca_gate_tb;
    ...
    initial begin
        ...
        $dumpfile({"results/power/rca_gate_", workload, ".vcd"});
        $dumpvars(0, rca_gate_tb);
        ...
        if (errors == 0)
            $display("PASS: RCA gate-level simulation completed with no errors.");
        else
            $display("FAIL: RCA gate-level simulation found %0d errors.", errors);
