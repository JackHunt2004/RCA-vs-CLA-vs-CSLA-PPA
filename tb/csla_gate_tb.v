...
module csla_gate_tb;
    ...
    initial begin
        ...
        $dumpfile({"results/power/csla_gate_", workload, ".vcd"});
        $dumpvars(0, csla_gate_tb);
        ...
        if (errors == 0)
            $display("PASS: CSLA gate-level simulation completed with no errors.");
        else
            $display("FAIL: CSLA gate-level simulation found %0d errors.", errors);
